...
final class SharedDatabase {
    ...
    // Issue #209: >0 while the keyboard owns the hot→cold editor harvest. The separate
    ...
    func current() -> LimeDB? {
        ...
        if suspensionCount > 0 { return nil }
    ...
    func setCurrent(_ datasource: LimeDB?) {
        lock.lock()
        guard suspensionCount == 0 || datasource == nil else {
            lock.unlock()
            try? datasource?.closeForReplacement()
            return
        }
