...
/// pings, so the app resolves to an active state (activeNoFullAccess / fullyEnabled);
/// otherwise it resolves to enabled-but-not-active and shows the 切換為萊姆輸入法 button.
///
...
final class SetupActivateLadderUITest: XCTestCase {
    ...
    func testForceEnabledResolvesLadderOffNotEnabled() throws {
        ...
        let activeBanner = app.staticTexts.containing(
            NSPredicate(format: "label CONTAINS %@", "已使用中")).firstMatch
        let activateButton = app.buttons["切換為萊姆輸入法"]
        ...
        XCTAssertTrue(resolved,
                      "ladder should resolve to active (已使用中) or enabled-not-active (切換為萊姆輸入法)")
