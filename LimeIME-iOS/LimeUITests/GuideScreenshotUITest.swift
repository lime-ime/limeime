...
/// naturally sit in enabledNotActive). Verifies the 切換為萊姆輸入法 button + 長按 🌐 hint.
final class GuideScreenshotUITest: XCTestCase {
    ...
    func testEnabledNotActiveShowsActivateGuide() throws {
        ...
        let button = app.buttons["切換為萊姆輸入法"]
        XCTAssertTrue(button.waitForExistence(timeout: 10),
                      "enabled-but-not-active should show the 切換為萊姆輸入法 activate button")
        XCTAssertTrue(app.staticTexts["長按 🌐 選擇萊姆輸入法"].exists,
                      "activate rung should show the 長按 🌐 選擇萊姆輸入法 hint")
