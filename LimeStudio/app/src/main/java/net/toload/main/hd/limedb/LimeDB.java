...
public class LimeDB  extends LimeSQLiteOpenHelper {
	...
	* Calling from onUpgrade with SquliteDataabase object to upgrade.
	...
	private boolean checkDBConnection(){
		...
		if(mLIMEPref.getMappingLoading()) {   //mapping loading in progress, database is not available for query
			if(DEBUG)
...
//			}else 
	...
	*  Jeremy '15,5,21
	...
	public void restoreUserRecords(final String table)
	{
		...
		//TODO:  put this into working thread?
			...
			if (cursor.moveToFirst()) {
				...
				try {
					for (String entry : codeList) {
						...
						//f(percentageDone>99) percentageDone = 99;
						//if(DEBUG)
