...
- **直向鍵盤模式（Android 手機與 iPhone）：**選擇標準、分離、靠左或靠右。分離模式方便雙手握持，靠左或靠右則方便單手操作（預設為標準）。數字鍵盤不提供分離模式。

- **橫向分離鍵盤（Android 手機與 iPhone）：**控制手機橫向時是否將鍵盤左右分割，與直向模式分開設定（預設為關閉）。

- **分離鍵盤（Android 平板與 iPad）：**選擇關閉、開啟或僅橫向開啟（預設為關閉）。此設定不會被手機的直向或橫向設定改寫。

- **數字鍵盤位置（Android 平板與 iPad）：**使用數字鍵盤時，可選擇滿版、靠左、靠右或置中（預設為滿版）。

> **也可從鍵盤選單快速調整**手機直向會顯示「直向鍵盤模式」，橫向會顯示「橫向分離鍵盤」；平板與 iPad 則依目前鍵盤顯示「分離鍵盤」或「數字鍵盤位置」。
