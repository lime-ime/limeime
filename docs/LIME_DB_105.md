...
**Architecture:** DB 105's `version < 105` branch runs the **full** idempotent schema set, and the version is stamped **only after it completes**, so `user_version = 105 ⟺ the DB is complete`. That guarantee is what makes the every-load net unnecessary: the bundled seed ships **complete at `user_version 105`** (regenerated by running `onUpgrade(104 -> 105)` on the previous 104 seed via the Android emulator), so a fresh install is complete with no first-open migration; a restore of any pre-105 backup, or an existing pre-105 install, re-runs the migration via `init()->migrate()` (iOS) / `SQLiteOpenHelper.onUpgrade` (Android); and no "stale 105" DB can exist because 105 is only reached through the migration or the complete seed. Goal 1's `imkeys` backfill for a table restored into an *already-105* live DB is covered by the IM-load-time fallback (export prologue) + the runtime in-memory `imKeysForTable`, so it too needs no per-load net.
...
- **Seed ships complete at `user_version 105`:** the bundled `lime.db` is regenerated by running the real `onUpgrade(104 -> 105)` on the previous 104 seed (Android emulator, minimal schema/keyboard set only — no emoji refresh, keeping it ~4 MB), then stamped 105. So a fresh install gets a complete DB with no first-open migration. (An earlier iteration kept the seed at 104 and relied on first-open migration; superseded — a complete seed is cleaner and matches the LIME_DB_104 convention of shipping the seed at the current version.)
...
- Bundled `lime.db` (both platforms, the shared `res/raw` file) **ships complete at `user_version 105`** — regenerated with the post-104 keyboards (`computernum` / `cangjie-semicolon` / `limenumsym2`) and the `tricode` table baked in. No new mapping rows or `im` rows are added (standard IM tables stay empty; the `imkeys` backfill is a no-op on the schema-only seed).
...
**Seed:** the bundled `lime.db` ships **complete at `user_version 105`** — regenerated by running the migration on the previous 104 seed, so it physically contains the post-104 keyboards + `tricode` table. A fresh install is complete immediately; the `< 105` migration exists only for in-place upgrades of pre-105 installs and restores of pre-105 backups.
...
- [x] **Regenerate the seed complete at `user_version 105`** (`LimeStudio/app/src/main/res/raw/lime.db`, the iOS `Copy lime.db to bundle` source, + the tracked `Database/lime.db` mirror). Generated via a one-off Android instrumentation run: raw-copy the 104 seed, call `onUpgrade(104 -> 105)` directly (schema/keyboards only, no emoji refresh → ~4 MB), stamp 105, `adb pull`, verify (105 + `computernum`/`limenumsym2`/`tricode`, standard IM tables still empty), install into both seed files.
...
- [x] Seed ships complete at `user_version 105` (shared file with iOS — regenerated on the Android emulator; see the iOS task). `SQLiteOpenHelper.onUpgrade` runs only for pre-105 upgrades/restores.
...
- [x] **Bundled seed is complete v105:** the bundled seed ships at `user_version 105` and already contains the post-104 `limenumsym2` keyboard and `tricode` table; opening it is a no-op migration. Proves a fresh install gets a complete 105 DB directly, with `ensureCurrentDatabase()` doing no schema work. (`testBundledSeedIsCompleteV105`; the in-place upgrade path is covered by `testInPlaceUpgrade104To105BackfillsDayiAndKeyboards`.)
