...
- Status: **OPEN / v6.1.23 popup fix delivered; v6.1.26 auto Chinese punctuation fix delivered; v6.1.27 delivered the wider candidate-strip X tap target plus related #124 Android usability follow-ups. `01disney` confirmed the X tap target improved on Google Play v6.1.27 and added that the bottom-input composing/reverse-lookup hints are less likely to cover typed output. Jeremy approved the expanded `123` long-press request as follow-up backlog `feat#N03`; remaining open scope is original-reporter popup/readability confirmation, `feat#N03` delivery, and the separate language-switch / keyboard-hide sizing product decision.** (reopened 2026-06-21)
...
- After `limeimetw` posted the Google Play closed-test v6.1.27 follow-up at https://github.com/lime-ime/limeime/issues/124#issuecomment-4849071167, `01disney` replied in https://github.com/lime-ime/limeime/issues/124#issuecomment-4851877127 that the X problem improved and is convenient now, and later edited the same comment to add that bottom-input composing/reverse-lookup hints are less likely to cover typed output in the new version. Treat this as a positive community datapoint for v6.1.27, not full original-reporter closure. The same reply asks whether the English-keyboard `123` long-press shortcut can also apply inside table/input-method keyboards such as Boshiamy, because that keyboard is visually similar to the English keyboard; Jeremy approved this as follow-up backlog `feat#N03` on 2026-07-01. `feat#N03` should extend long-press `123` to IM keyboards/layouts that have an independent `123` key, excluding phone-family layouts, while preserving normal tap behavior.
- `01disney` later edited https://github.com/lime-ime/limeime/issues/124#issuecomment-4852013160 with screenshots reinforcing the separate Boshiamy language-switch / keyboard-hide sizing feedback: the `中` / `EN` switch key sits close to the larger keyboard-hide key, fast thumb input can hit the hide key by mistake, and visible key borders may make the keyboard easier to use. Treat this as adjacent layout/usability product feedback, not evidence that the v6.1.27 X tap-target fix or popup-position fix failed. Recommended next step is a maintainer/Jeremy decision whether to split a separate language-switch / keyboard-hide key sizing item or keep it for later keyboard-layout review.
