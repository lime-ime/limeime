...
- `python3 scripts/test_issue_209_ios_editor_refresh_lifecycle.py` — 32 tests, `OK`. Five are real-SQLite behavior tests; the source contracts cover quiesce/request ordering, bounded descriptor-owned ownership, deferred keyboard contention, process-local single-flight, shared request deadlines, safe recovery, suspension-gate enforcement, attach/commit/detach/close ordering, receipt ordering, both editor paths and retry removal.
...
The final review-fix code was validated twice on its exact pushed SHA:

- Run 38, SHA `605051e51d2da35bd36e76a3095fcfd42cca0170`: iOS tests passed; iOS archive passed.
- Run 39, SHA `76dca52d293228539cfba405a7a20af666a59bff`: iOS tests passed; iOS archive passed; no required failures; production workflow restored.

Native coverage includes real same-process lock contention, Related-versus-normal editor single-flight, cold suspension/reopen, ownership-reacquisition recovery, bounded persistent SQLite contention, hot→cold harvest and post-receipt cold writability. Tests use direct datasource state and real database access rather than WAL-sidecar lifetime as a connection proxy.
