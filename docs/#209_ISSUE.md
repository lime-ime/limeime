...
6. The retry-only workaround is gone: `editorRefreshBusyRetryWindow`, `editorRefreshBusyRetryBackoff`, `isTransientLockError`, `harvestEditorRefreshAttempt` and `scripts/test_issue_209_ios_editor_refresh_retry.py` are removed. The normal 5-second busy timeout remains for independent hot-side learning/commit writes. Normal Settings reopening waits for ownership; after the shared request deadline, failure recovery may reopen cold best-effort rather than leave all Settings readers empty until restart.
...
- `python3 scripts/test_issue_209_ios_editor_refresh_lifecycle.py` — 40 tests, `OK`. Five are real-SQLite behavior tests; the source contracts cover quiesce/request ordering, bounded descriptor-owned ownership, defensive unlock cleanup, observable timeout budgets, deferred keyboard contention, malformed-request isolation, cancellation-safe polling, process-local single-flight, shared request deadlines, unlock-before-signal ordering, safe recovery, suspension-gate enforcement, attach/commit/detach/close ordering, receipt ordering, both editor paths and retry removal.
...
The final review4 code was validated on its exact pushed SHA:
...
The documentation update that records Run 42 necessarily has a later commit SHA; production and test code are identical to the validated Run 42 tree.

Native coverage includes real same-process lock contention, controller-propagated initial/reacquisition timeout budgets, Related-versus-normal editor single-flight, cold suspension/reopen, ownership-reacquisition recovery, bounded persistent SQLite contention, hot→cold harvest and post-receipt cold writability. Tests use direct datasource state and real database access rather than WAL-sidecar lifetime as a connection proxy.
