...
- `python3 scripts/test_issue_209_ios_editor_refresh_lifecycle.py` — 29 tests, `OK`. Five are real-SQLite behavior tests; the source contracts cover quiesce/request ordering, bounded descriptor-owned ownership, process-local single-flight, safe recovery, suspension-gate enforcement, attach/commit/detach/close ordering, receipt ordering, both editor paths and retry removal.
