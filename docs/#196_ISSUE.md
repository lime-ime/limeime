...
Android-parity tests in `KeyboardViewControllerTest` confirmed the gap on the booted iPhone 17 Pro Max simulator. An end-to-end behavioral test drives the real `onKey()` handler through a `#if DEBUG` same-file test seam (no production visibility widening; `searchServer` left nil to model the missing combined-code candidate) and asserts the composing buffer holds exactly one appended root: it fails RED against the pre-fix source with `mComposing == "v,,"` (the caller fell through to `handleCharacter` and double-inserted) and passes GREEN with the fix at `"v,"`. Two source-parity tests pin the consume-once and refresh-strip contracts, and a smaller behavioral test shows the declared root is re-accepted into composing. The iOS correction mirrors Android — consume the appended root once and call `updateCandidates()` when nothing commits — after which all parity tests pass GREEN and the existing end-key routing/policy tests continue to pass. Reporter/device retest of a newer build remains required before closing #196.
