...
- Status: closed/source-fixed by maintainer merge on 2026-07-25; reporter-confirmed fixed on Android v6.1.37 on 2026-07-26
- Reporter: `ejmoog`
...
Verified on a Pixel 9 Pro API 37 emulator and the iPhone 17 Pro Max simulator: production and test sources compile; each new regression fails RED against the pre-correction code and passes GREEN with it; and the adjacent declared-root and punctuation end-key regressions continue to pass. Issue #196 remains closed. The reporter subsequently confirmed the original Android punctuation end-key path on v6.1.37; broader iOS and declared-root release QA remain separate from that confirmation.
...
Confirmed affected and reporter-confirmed fixed. The reporter tested Android 6.1.36, and the Android source path reproduces the routing error. The Android-only source fix merged as `a96dcef659aa20796f6cab2edefe091de24823de`. The adjacent declared-root stale-candidate blocker was subsequently corrected in the #206 follow-up and verified on a Pixel 9 Pro API 37 emulator. Both fixes are delivered in the verified v6.1.37 GitHub testing APK. After the targeted request to verify `v` then `,` commits `好，` without stale candidates, the reporter confirmed the issue was resolved.
...
Post-merge release QA and reporter confirmation:

5. Reporter-confirmed on Android v6.1.37: with alphabetic `imkeys` and `limeendkey=,.`, the requested `v` followed by `,` → `好，` behavior is resolved. The separate explicitly declared punctuation-root path still has only internal emulator/test coverage.
6. Confirm a table without `limeendkey` retains ordinary comma/period composition behavior.
7. Reporter retest was requested against verified Android v6.1.37 at https://github.com/lime-ime/limeime/issues/196#issuecomment-5079729489 and confirmed fixed at https://github.com/lime-ime/limeime/issues/196#issuecomment-5081295835. Closing acknowledgement: https://github.com/lime-ime/limeime/issues/196#issuecomment-5081298915.
