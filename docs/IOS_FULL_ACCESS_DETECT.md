...
`SetupTabView.swift` `refreshStatus()` reads one source:

- The system `AppleKeyboards` array (list of enabled keyboard bundle IDs), read from `UserDefaults.standard` and the `.GlobalPreferences` domain.
- It does not use `UITextInputMode.activeInputModes` private KVC. That path can terminate the app with an uncaught Objective-C exception on foreground and is not needed because active-keyboard detection is handled by the relay probe.

`keyboardEnabled = SetupDetection.keyboardEnabled(appleKeyboards:forceEnabled:)`, refreshed by a 1-second poll while the tab is visible.
