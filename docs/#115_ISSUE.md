...
- Android APK `LIMEHD2026-6.1.20.apk` now contains the startup keyboard-snapshot fix (GitHub Contents blob SHA `cbe1ff21ab7a499eef952c702ee5eb0a40131c05`, size 14,053,640 bytes), and the reporter retest request was posted in https://github.com/lime-ime/limeime/issues/115#issuecomment-4715747519. Keep the issue open pending reporter confirmation.
...
Open / pending Android reporter retest. Labeled `bug` + `Usability`, assigned to `jrywu`, and tracked in `docs/BACKLOG.md` under active issue follow-up. PR #116 / merge commit `0a03fcca34fd` brought in the startup keyboard-snapshot fix, and Android APK `LIMEHD2026-6.1.20.apk` contains it (GitHub Contents blob SHA `cbe1ff21ab7a499eef952c702ee5eb0a40131c05`, size 14,053,640 bytes). Retest request posted in https://github.com/lime-ime/limeime/issues/115#issuecomment-4715747519. Revised live issue scope after the 2026-06-14 edit: tested first mounted non-`注音` IMs (`倉頡`, `大易`, `行列`, `行列10`) can start with the wrong Chinese-mode generic keyboard; adding a second IM can make any active IM, including `注音`, show the same wrong first keyboard; manual Array10 `.lime` import can still intermittently choose the wrong default layout. Current retest asks the reporter to verify those three paths on 6.1.20; keep the issue open until reporter or maintainer confirmation.
