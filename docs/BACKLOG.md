...
- fix#160 iOS: the shared-catalog keyboard option `limenumsym` (`LIME+數字符號鍵盤`) renders the ordinary number-row QWERTY layout instead of its dedicated number/symbol layout. Android is reporter-confirmed working. PR #162 merged the missing phone/iPad resources to `master` as `c56593f8e3fd76b4a800b66b12ab76b7a6b96f46`, and Xcode Cloud run 16 succeeded for the PR head. Release remains blocked because an independent semantic audit found the merged iPad generator output loses required punctuation (`_` on full iPad Shift, `_` and `+` on narrow iPad Shift, and `=` on normal narrow iPad) while the generator-relative test still passes. Add source-independent RED assertions, correct and regenerate the iPad variants in a follow-up PR, then complete device verification and release. Android is not in scope. See `docs/#160_ISSUE.md`.
