...
- fix#158 Android: remove synchronous SQLite metadata reads from `LIMEService.handleEndkeyCommit()`. Android Vitals recorded the v6.1.30 IME main thread waiting in `SQLiteConnectionPool` during a key-up event. Cache the active IM's exact `limeendkey` and stored `imkeys` values before the first key reaches this path, refresh them on IM/import/metadata/restore changes, preserve phonetic/custom behavior, and verify the key touch path performs no database access. The analogous iOS path already uses a per-IM metadata cache and is not affected by this Android SQLite ANR. See `docs/#158_ISSUE.md`.
