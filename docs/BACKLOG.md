...
- #128 Android: Samsung A55 / Android 16 / One UI 8.5 report says `喜好設定 / 打字震動` is enabled and Samsung `觸控震動` is enabled, but soft-key presses produce no vibration while LIME `打字音效` works. Bug-fix PR workflow adds an Android 12+ fallback from failed `performHapticFeedback(KEYBOARD_TAP)` to direct `Vibrator.vibrate(...)` tagged as `USAGE_TOUCH`; ask the reporter to retest only after a newer APK/Play build contains the merged haptic-feedback change.
