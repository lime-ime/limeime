...
## Pending fixes
- fix#161 Android/iOS: PR #167 merged the escaped `pword` prefix management search and iOS runtime parity, but post-merge review found that the newly added Android runtime related cache is invalidated by automatic learning only, not by manual `關聯字管理` add/update/delete operations. Add focused invalidation coverage and correct both initial/full cache keys before the next Android build. iOS still needs corrected-source XCTest/Xcode Cloud validation, a newer build, and reporter confirmation. Current Android APK v6.1.32 predates the merge. See `docs/#161_ISSUE.md`.
