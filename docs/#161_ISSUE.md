...
- State: closed as completed after the reporter confirmed that Android GitHub APK v6.1.34 refreshes the filtered management list correctly after deletion; iOS validation and delivery remain separate release QA
...
- The recording established a separate Android management-list synchronization defect. With search text `/`, the list initially showed three records with the same parent `/084V9P2` and children `/084V9P2`, `台中`, and `/`. After a delete was confirmed, the filtered list count changed but a visible row could retain the deleted record's text; tapping that stale-looking row opened a different surviving record. Clearing and re-entering the search refreshed the visible row contents. This did not invalidate the confirmed prefix-search or keyboard-candidate cache fixes, but it kept #161 open for a focused list-refresh fix until the v6.1.34 retest.
...
- GitHub Release v6.1.33 targets `b2fa71779d8423b92896fa1a0262706bb62ea4fa`, which is an ancestor of the PR #168 merge and therefore predates this deletion-list fix. Reporter retesting of this follow-up therefore waited until Android GitHub APK v6.1.34 included `285b9fde57384203c074f9b16094f2bdc757a3c6`.
- GitHub Release v6.1.34 targets `d45aa437b6356bfef5079ceebbfcd8d295a300b8` and contains PR #168. Its verified GitHub testing-track asset is `LIMEHD2026-6.1.34.apk` (7,112,576 bytes, SHA-256 `d16d7fde5d634d655148396c657e8ffab5f3868f434f705f9568855da4e3e84f`) at https://github.com/lime-ime/limeime/releases/download/v6.1.34/LIMEHD2026-6.1.34.apk. The targeted deletion-list retest request was https://github.com/lime-ime/limeime/issues/161#issuecomment-5016727224.
- Reporter `coral0819` confirmed the Android v6.1.34 retest in https://github.com/lime-ime/limeime/issues/161#issuecomment-5018118700: after deleting a related record, the list immediately showed the correct remaining data and tapping an item opened the record matching its visible text. The project account added a thumbs-up reaction, posted the closing acknowledgement at https://github.com/lime-ime/limeime/issues/161#issuecomment-5018122965, and closed the issue as completed. This confirmation is limited to the Android GitHub APK behavior requested in the retest.
...
- [x] Reporter confirms in Android GitHub APK v6.1.34 that deletion refreshes visible row contents and the opened record matches the selected row
