...
| A4 | (this) | **Second PR #223 merge-review blocker fixed**: install-only lineage (fresh install, cold never restored → NO `epoch_uuid`; hot carries a random bootstrap `epoch_uuid` and NO `applied_epoch` — scanAndApply §4.3 deliberately removes the marker for a nil-epoch lineage and treats nil==nil as converged). `capturePendingLearning` collapsed the two values with `appliedEpoch ?? hotEpoch`, so the flush compared cold(nil) against hot(bootstrap-UUID) and rejected EVERY flush — outbox never drained, learning never reached cold, A2 gate permanently read-only. Fix: capture both values raw; the flush's in-cold-transaction guard now applies scanAndApply's exact two-term convergence rule (`coldEpoch == appliedEpoch OR coldEpoch == hotEpoch`). TDD: `testInstallOnlyLineageFlushDeliversWithNilColdEpoch` observed RED pre-fix; inverse guard `testRestoredColdEpochStillRejectsInstallOnlyOutbox` (restored non-nil cold epoch still writes/acks nothing) GREEN both sides. One existing fixture corrected (assertion unchanged): `testFlushRequiresProtocolMarkerAndMatchingEpochBeforeAck` phase 2 modeled "stale lineage" as stale `applied_epoch` with hot self-marked on cold's epoch — which is §4.3's converged interrupted-restore-stamp fast-path (correctly delivers under the mirrored rule); the fixture now sets BOTH hot `epoch_uuid` and `applied_epoch` to the old lineage, the genuine mismatch its assertion describes. Also hardened the A3 seam tests for CI (60s waits — Xcode Cloud .utility-QoS starvation timed out the 10s waits; lazy learn_outbox creation made the follow-on read a SQL error instead of a clean assertion). Full suite gated before push per maintainer instruction. |
| A5 | (this) | **Third PR #223 merge-review blocker fixed**: the unmarked incremental deletion paths (keyboard-first mixed-version upgrade — old app deletes an IM, published snapshot loses the table + its rev) dropped the hot table and its `rev:` metadata but left that table's `learn_outbox` rows. Such rows are permanently undeliverable — `capturePendingLearning` skips rows whose table is gone, `acknowledge` only deletes captured items, but `pendingLearningCount` counts every raw row — so `pend` never reached 0 and the A2 gate stayed locked forever. Fix: `deleteOutbox(table:in:)` added to both unmarked drop transactions, plus an orphan sweep in `flushPendingLearning` (under the flush lock, before capture) that removes rows referencing nonexistent hot tables — needed because devices that ran the pre-fix path already carry orphans no drop-site fix can reach. TDD: both regressions observed RED pre-fix — `testUnmarkedSnapshotTableRemovalClearsPendingOutbox` (the exact keyboard-first mixed-version path, FA-off so it proves the drop-site cleanup, not the sweep) and `testFlushSweepsOrphanedOutboxRowsFromMissingTables` (pre-existing orphan + valid row: orphan swept, valid delivered). Full suite + Linux gate before push. |
| A6 | (this) | **Fourth PR #223 merge-review blocker fixed**: A5 fixed the unmarked drop branches but not the sibling unmarked COPY branch — a wholesale table copy removes a hot-only learned row while its `learn_outbox` entry survives. Capture skips a missing logical ROW (not just a missing table), the A5 sweep was table-level only, and `pendingLearningCount` counts raw rows → stranded entry, `pend` > 0 forever, A2 gate permanently read-only on the keyboard-first mixed-version path. Fix: new `deleteUncapturableOutbox(table:in:)` (one `DELETE … WHERE NOT EXISTS` mirroring capture's row probe, incl. the unsafe-name and missing-table guard paths) called (a) atomically inside the unmarked copy transaction and (b) per pending table in the flush's pre-capture sweep, which now supersedes the A5 table-level sweep as the catch-all for state left by older builds. §4.4 documents the pre-capture sweep per review requirement. TDD: both regressions observed RED pre-fix — `testUnmarkedCopyClearsOutboxRowsWithoutSurvivingHotRow` (FA-off, proves the copy-site cleanup) and `testFlushSweepsOutboxEntriesWithMissingHotRows` (stranded entry swept, valid entry delivered). Full suite + Linux gate before push. |
