...
So `et_41` was the only phonetic type whose visible layout depended on the fragile config round-trip instead of the preference. Its siblings `eten26` / `hsu` (and all of Android) read the preference directly and were never affected.

## Fix (landed)

`LimeIME-iOS/LimeKeyboard/KeyboardViewController.swift`

- Added a pure helper `phoneticSpecialLayoutId(for kbType:)` that maps the `phonetic_keyboard_type` preference to its dedicated visible layout id, mirroring Android's `resolvePhoneticKeyboardCode()`:
  - `et_41` / `eten` → `lime_et_41`  ← the fix for #156
  - `eten26*` / `et26` → `lime_et26`
  - `hsu*` → `lime_hsu`
  - otherwise → `nil` (standard; resolved via the keyboard-config path)
- `resolvedLayoutId(for:)` now calls that helper for the phonetic IM, so the visible layout is driven straight from the preference. `et_41` no longer depends on the `im.keyboard` config round-trip. Behavior for `eten26` / `hsu` is unchanged (the previous inline branches were consolidated into the helper verbatim).

The Settings-side write (`updatePhoneticKeyboard` → `setImConfigKeyboard`) and `refreshPhoneticKeyboardPrefs()` are left in place; they are now redundant for the phonetic visible layout but still keep the persisted config and the detail-page label consistent.
