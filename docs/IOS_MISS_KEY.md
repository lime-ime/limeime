...
### 2026-07-15 — Dead taps on inter-key GAPS: transparent-pixel gate, not starvation (FIXED)

A fourth dead-tap trigger, distinct from the three above and **not load-dependent**: taps in the gaps / edges / margins **between** keys did nothing — no highlight, no haptic, no character (any typing speed). The `KeyTouchLayer` `hitTest` already *claims* transparent gaps (§2026-07-02), but that is necessary, not sufficient: iOS's custom-keyboard touch gate **drops the touch on a fully transparent pixel before `hitTest` runs** ([IOS_CANDI_TOUCH.md §Resolution](IOS_CANDI_TOUCH.md)), and the layer used a `.clear` background. On-key taps always worked (key frames have solid backgrounds) — the same control case as the candidate bar.

**Fix (commit `7188a4ae`):** `KeyTouchLayer` background → `LayoutMetrics.TouchTrap.fill` (`white 0.5, alpha 0.01`, non-transparent so the gate passes the touch to `hitTest`); per-row touch layer widened to the **full row width** (keys centered via a layout guide) so indented-row margins resolve to the edge key by proximity; same fill on the expanded candidate panel's scroll content + cells. Regression test in `TouchLayerGestureTests`.

Four now-known dead-tap triggers, same end state (press never reaches the button), distinct causes: (1) delayed/dropped `touchesBegan` under load → single-owner layer + `delaysTouchesBegan=false` (§2026-07-02); (2) screen-edge system-gesture hold → drive feedback off a recognizer (§2026-06-30); (3)/(4) **transparent-pixel gate on the between-key gaps → `TouchTrap.fill` (this entry).** Layout-author framing: [ANDROID_IPHONE_KEYBOARD.md §Touch Handling](ANDROID_IPHONE_KEYBOARD.md), [IPAD_KEYBOARD.md §4.2.7](IPAD_KEYBOARD.md).
