...
2. **Own, hand cold over, then request (#209).** Settings takes the cross-process
   `EditorRefreshFileLock`, then
   ...
   `{requestUUID, table, expiresAt}`, rings `tables.updated`, and **polls** for
   `outbox/editor.refresh.receipt.json` whose `requestUUID` matches, up to
   `editorRefreshPollTimeout`. Any Settings-side cold access left open here fails the harvest with
   `SQLite error 5: database is locked`. The view therefore also holds its FIRST cold load until
   this call returns. It then releases ownership so the keyboard can acquire it.
...
4. **Reacquire, reopen, unlock.** `refreshTableFromKeyboard` reacquires cross-process ownership
   ...
   keyboard already owns an in-flight harvest, Settings waits for ownership instead of reopening
   cold underneath it, then accepts any matching terminal receipt that landed meanwhile. Full
...
| Editors — probe summon, `同步中`, `beginEditorSession`, `publishEditorCloseIfNeeded`, `scenePhase == .background` commit, `@EnvironmentObject RelayActiveState` | `LimeSettings/Views/RecordListView.swift`, `RelatedListView.swift`, `IMDetailView.swift` |
